<nav class="sticky top-0 z-50 flex justify-between p-5 align-center bg-white border-2 border-white border-b-gray-100">
    <h1 class="text-3xl font-bold flex justify-start align-center py-2 w-96">{{$title}}</h1>
    
    @if($title != "Account" && $title != "Citizen" && $title != "Barangay" && (!isset($assignbrgy) || $title != $assignbrgy) && (!isset($assignbrgyadmin) || $title != $assignbrgyadmin))
    <div class="flex items-center justify-end space-x-4 flex-1"> <!-- Adjusted this div -->
        <form action="/search" method="post" class="flex gap-x-3 items-center">
            @csrf
            <label class="relative flex items-center">
                <span class="sr-only">Search</span>
                <span class="absolute inset-y-0 left-0 flex items-center pl-2">
                    <span class="material-symbols-outlined text-slate-400">search</span>
                </span>
                <input name="searchvalue" value="{{ $searchValue ?? '' }}" class="placeholder:italic placeholder-text-slate-400 block bg-white w-full border border-slate-300 rounded-md py-2 pl-9 pr-3 shadow-sm focus:outline-none focus:border-sky-500 focus:ring-sky-500 focus:ring-1 sm:text-sm" placeholder="Search Here" type="text" name="search"/>
            </label>
            <button type="submit" class="flex items-center font-medium text-slate-100 bg-[#BC486B] hover:bg-[#82083C] rounded-lg py-2 px-8">Search</button>
        </form>
        
        <ul class="flex space-x-4">
            <li>
                <a href="/add_citizen">
                    <button class="flex items-center font-medium text-slate-100 bg-[#BC486B] hover:bg-[#82083C] rounded-lg py-2 px-8">
                        <span class="material-symbols-outlined">add</span> Add New
                    </button>
                </a>
            </li>
        </ul>
    </div>
    @endif

</nav>
