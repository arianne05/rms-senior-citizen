        ...
        <div class="container flex flex-col w-full h-auto p-10">
            <form action="/process_edit_user/{{$users->id}}" id="myForm" method="post">
                ...
                <label class="block mb-5">
                    ...
                    <input type="text" name="email" value="{{$users->email}}" class="mt-1 px-3 py-2 bg-white border shadow-sm border-slate-300 placeholder-slate-400 focus:outline-none focus:border-sky-500 focus:ring-sky-500 block w-full rounded-md sm:text-sm focus:ring-1" placeholder="you@example.com"/>
                    @error('email')
                        <p class="text-red-500 text-xs p-2">
                            {{$message}}
                        </p>
